...
- **Header SearchBar Centering**: SearchBar now properly centered on desktop layout using CSS Grid
...
- **Odesli Rate Limiting**: Removed redundant client-side 6s delay (server already enforces rate limiting)

## [0.31.0] - 2025-12-11

### Added

- **Server-Side Streaming Links Cache**: Memory cache with disk persistence (7-day TTL) reduces Odesli API calls across users
- **Streaming Links Prefetch**: Background prefetch now fetches streaming links during release group prefetch
- **Separate Odesli Rate Limiter**: Dedicated 6-second rate limiter for Odesli API (separate from MusicBrainz 2s limiter)

### Fixed

- **UserCountry Validation**: Cache key now includes `userCountry` to prevent serving wrong region's links

## [0.30.3] - 2025-12-11

### Fixed

- **Prefetch Network Errors**: Add retry logic with exponential backoff (1s→2s→4s, max 3 retries) for ECONNRESET errors
...
- **Prefetch Queue Restart**: Fixed background prefetch restarting when viewing album details
  - Singletons now persist across Next.js module re-evaluations using `globalThis`

## [0.30.0] - 2025-12-11

### Fixed

- **Prefetch Queue Not Starting**: Fixed critical bug where background prefetch never started
  - Removed backwards `isPrefetching()` check that blocked queue initialization
  - Queue now starts correctly when items are added to prefetch queue

## [0.29.0] - 2025-12-11

### Added

- **Cache Persistence (Phase 7)**: Server-side cache now persists across restarts
  - Automatic persistence every 5 minutes
  - Graceful shutdown handlers (SIGTERM, SIGINT) to save cache before exit
  - Prevents loss of hours of prefetch work on server restart

## [0.28.0] - 2025-12-11

### Added

- **Cache Statistics & Monitoring (Phase 6)**: Real-time visibility into cache performance
  - Created [api/cache/stats/route.ts](../src/app/api/cache/stats/route.ts) - API endpoint for cache statistics
  - Updated [app/config/page.tsx](../src/app/config/page.tsx) - Added Cache Status section with real-time stats
  - Displays cache hit rates, memory usage, and prefetch progress
  - Auto-refreshes every 10 seconds when prefetch is running
  - Server-side cache implementation now complete (Phases 1-6)
...
- **Collection Refresh Button**: Prevents multiple simultaneous refresh operations with loading state and animation
  - Refresh button now disabled during refresh operation
  - Spinning animation provides visual feedback

## [0.22.1] - 2025-11-28

### Fixed

- **Release Year Display**: Fixed incorrect year showing from later releases instead of original first release date
